...
import json
import sys
from bottle import static_file, SimpleTemplate
...
def present_welcome():
    ...
    return bottle.template("welcome", root ='static/',{'username':username})        
...
def newpost():
    ...
    return bottle.template("newpost", {'username':username})        
...
def process_post():
    ...
    if subject or content == "":
            errors['content_error'] = "Please fill both field"
            return bottle.template("newpost.tpl", errors)
    else:
        response.redirect("")
